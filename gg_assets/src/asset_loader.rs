...
pub enum LoadResult {
    ...
    Mesh {
        handle: Uuid,
        data: Result<gg_renderer::Mesh, String>,
    },
    SkinnedMesh {
        handle: Uuid,
        data: Result<gg_renderer::GltfSkinData, String>,
    },
}
    ...
    loop {
        ...
        match request {
            ...
            Ok(LoadRequest::SkinnedMesh { handle, path }) => {
                let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    gg_renderer::load_gltf_skinned(&path).map_err(|e| e.to_string())
                }));
                let data = match result {
                    Ok(data) => data,
                    Err(_) => Err(format!(
...
fn load_gltf_merged(path: &std::path::Path) -> EngineResult<gg_renderer::Mesh> {
    let meshes = gg_renderer::load_gltf(path)?;
    if meshes.len() == 1 {
    ...
    let name = path
        ...
        .unwrap_or("mesh")
        .to_string();
    Ok(gg_renderer::Mesh::merge(meshes, name))
}
