...
impl Renderer {
    ...
    pub fn update_render_pass(&mut self, render_pass: vk::RenderPass) {
    ...
    pub fn use_offscreen_pipeline(&mut self, use_offscreen: bool) {
        if let Some(data) = &mut self.renderer_2d {
    ...
    pub fn init_2d(&mut self) -> EngineResult<()> {
    ...
    pub fn begin_scene(&mut self, camera_vp: Option<&Mat4>, ctx: DrawContext) {
    ...
    pub fn emit_particles(&mut self, props: &crate::ParticleProps) {
        if let Some(ps) = &mut self.gpu_particles {
            ps.emit(props);
        }
    }
