...
impl Framebuffer {
    ...
    pub fn clear_values(&self, clear_color: [f32; 4]) -> ClearValues {
    ...
    pub fn read_pixel_result(&mut self, current_frame: usize) {
        let ptr = self
            .readback_allocation
            .mapped_ptr()
    ...
    pub fn color_attachment_image(&self, index: usize) -> vk::Image {
        self.color_attachments[index].image
    }

    /// Take the pending readback request (resets it to None).
    pub fn take_pending_readback(&mut self) -> Option<(usize, i32, i32)> {
        self.pending_readback.take()
    }

    /// Get the readback staging buffer handle.
    pub fn readback_buffer(&self) -> vk::Buffer {
        self.readback_buffer
    }

    pub fn set_egui_texture_id(&mut self, id: egui::TextureId) {
