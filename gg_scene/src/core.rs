use gg_core::cursor::CursorMode;
use gg_renderer::VertexArray;
...
pub struct SceneCore {
    ...
    pub uuid_cache: HashMap<u64, hecs::Entity>,
    ...
    pub id_cache: HashMap<u32, hecs::Entity>,
    ...
    pub name_cache: RwLock<Option<HashMap<String, u64>>>,

    // Deferred operations
    /// Deferred entity destruction queue (UUIDs). Flushed after script callbacks.
    pub pending_destroy: Vec<u64>,
    ...
    pub va_graveyard: VecDeque<Vec<VertexArray>>,
    ...
    pub transform_cache: RwLock<HashMap<hecs::Entity, glam::Mat4>>,
    ...
    pub transform_snapshots: RwLock<HashMap<hecs::Entity, (glam::Mat4, Option<u64>)>>,

    // Asset resolution tracking
    /// When `true`, all texture handles have been resolved.
    pub textures_all_resolved: bool,
    ...
    pub shadow_cascade_cache: RwLock<Option<([glam::Mat4; 4], [f32; 3], f32, [f32; 4])>>,
    ...
    pub script_module_search_path: Option<PathBuf>,
    ...
    pub save_data_directory: Option<PathBuf>,
    ...
    pub render_buffers: Mutex<RenderBufferPool>,
}
...
impl SceneCore {
    ...
    pub fn request_dead_zone(&self, axis: gg_core::events::gamepad::GamepadAxis, value: f32) {
        self.requested_dead_zones.lock().push((axis, value));
    }
    /// Take (drain) all pending dead zone change requests.
    pub fn take_requested_dead_zones(&self) -> Vec<(gg_core::events::gamepad::GamepadAxis, f32)> {
