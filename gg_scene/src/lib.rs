...
pub struct Scene {
    ...
    pub physics_world: Option<PhysicsWorld2D>,
    #[cfg(feature = "physics-3d")]
    pub physics_world_3d: Option<PhysicsWorld3D>,
    #[cfg(feature = "lua-scripting")]
    pub script_engine: Option<ScriptEngine>,
    pub audio_engine: Option<audio::AudioEngine>,
}
...
macro_rules! for_each_addable_component {
    ($callback:ident) => {
        $callback!(
            ($crate::CameraComponent, "Camera"),
            ($crate::SpriteRendererComponent, "Sprite Renderer"),
            ($crate::CircleRendererComponent, "Circle Renderer"),
            ($crate::SpriteAnimatorComponent, "Sprite Animator"),
            (
                $crate::InstancedSpriteAnimator,
                "Instanced Sprite Animator"
            ),
            (
                $crate::AnimationControllerComponent,
                "Animation Controller"
            ),
            ($crate::TextComponent, "Text"),
            ($crate::RigidBody2DComponent, "Rigidbody 2D"),
            ($crate::BoxCollider2DComponent, "Box Collider 2D"),
            (
                $crate::CircleCollider2DComponent,
                "Circle Collider 2D"
            ),
            ($crate::TilemapComponent, "Tilemap"),
            ($crate::AudioSourceComponent, "Audio Source"),
            ($crate::AudioListenerComponent, "Audio Listener"),
            ($crate::ParticleEmitterComponent, "Particle Emitter"),
            // 3D colliders handled manually in editor for mesh-aware defaults.
        );
        #[cfg(feature = "physics-3d")]
        $callback!(
            ($crate::RigidBody3DComponent, "Rigidbody 3D"),
        );
        $callback!(
            ($crate::MeshRendererComponent, "Mesh Renderer"),
            (
                $crate::DirectionalLightComponent,
                "Directional Light"
            ),
            ($crate::PointLightComponent, "Point Light"),
            ($crate::AmbientLightComponent, "Ambient Light"),
            ($crate::EnvironmentComponent, "Environment Map"),
            ($crate::UIAnchorComponent, "UI Anchor"),
            ($crate::UIRectComponent, "UI Rect"),
            ($crate::UIImageComponent, "UI Image"),
            ($crate::UIInteractableComponent, "UI Interactable"),
            ($crate::UILayoutComponent, "UI Layout"),
        );
    };
}
...
mod tests {
    ...
    fn find_asset_references_sprite() {
        let mut scene = Scene::new();
        let handle = gg_core::uuid::Uuid::from_raw(42);
    ...
    fn find_asset_references_audio() {
        let mut scene = Scene::new();
        let handle = gg_core::uuid::Uuid::from_raw(99);
    ...
    fn find_asset_references_tilemap() {
        let mut scene = Scene::new();
        let handle = gg_core::uuid::Uuid::from_raw(77);
    ...
    fn find_asset_references_none_when_unused() {
        ...
        let handle = gg_core::uuid::Uuid::from_raw(123);
        let refs = scene.find_asset_references(handle);
    ...
    fn find_asset_references_multiple_entities() {
        let mut scene = Scene::new();
        let handle = gg_core::uuid::Uuid::from_raw(55);
