...
use gg_core::events::gamepad::{GamepadAxis, GamepadButton};
use gg_core::events::{KeyCode, MouseButton};
use gg_core::input::Input;
    ...
    with_entity_mut(lua, entity_id, false, |scene, entity| {
            ...
            scene.get_component_mut::<super::SkeletalAnimationComponent>(entity)
        {
            sac.clip_events
                .entry(clip_name)
                .or_default()
                .push(gg_renderer::skeleton::SkeletalAnimationEvent {
                    time,
                    name: event_name,
                });
            true
...
fn lua_set_sprite_texture(lua: &Lua, (entity_id, handle_raw): (u64, u64)) -> LuaResult<()> {
    with_entity_mut(lua, entity_id, (), |scene, entity| {
        if let Some(mut s) = scene.get_component_mut::<super::SpriteRendererComponent>(entity) {
            s.texture_handle = gg_core::uuid::Uuid::from_raw(handle_raw);
            s.texture = None;
        }
    })
}
...
fn lua_set_cursor_mode(lua: &Lua, mode_str: String) -> LuaResult<()> {
    let mode = match mode_str.as_str() {
        "normal" => gg_core::cursor::CursorMode::Normal,
        "confined" => gg_core::cursor::CursorMode::Confined,
        "locked" => gg_core::cursor::CursorMode::Locked,
        _ => {
...
fn lua_get_cursor_mode(lua: &Lua, _: ()) -> LuaResult<String> {
    ...
    let s = match scene.cursor_mode() {
        gg_core::cursor::CursorMode::Normal => "normal",
        gg_core::cursor::CursorMode::Confined => "confined",
        gg_core::cursor::CursorMode::Locked => "locked",
    };
    Ok(s.to_string())
}
...
fn lua_set_ui_image(lua: &Lua, (entity_id, tex_handle): (u64, u64)) -> LuaResult<()> {
    ...
    if scene.has_component::<super::UIImageComponent>(entity) {
        if let Some(mut img) = scene.get_component_mut::<super::UIImageComponent>(entity) {
            img.texture_handle = gg_core::uuid::Uuid::from_raw(tex_handle);
            img.texture = None;
        }
    } else {
        scene.add_component(
            ...
            super::UIImageComponent {
                texture_handle: gg_core::uuid::Uuid::from_raw(tex_handle),
                ..Default::default()
            },
        );
    }
...
fn lua_save_data(lua: &Lua, (slot, data): (String, LuaTable)) -> LuaResult<bool> {
    ...
    match gg_core::platform_utils::atomic_write(&save_path, &json_string) {
        Ok(()) => {
