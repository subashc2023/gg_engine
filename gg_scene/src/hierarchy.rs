...
impl SceneCore {
    ...
    pub fn build_world_transform_cache(&self) {
    ...
    fn build_world_transform_cache_impl(&self) -> HashMap<hecs::Entity, glam::Mat4> {
        ...
        if entity_count < gg_core::jobs::parallel::PAR_THRESHOLD {
        ...
        let sub_results: Vec<Vec<(hecs::Entity, glam::Mat4)>> = gg_core::jobs::pool().install(|| {
            roots
                .par_iter()
                .map(|&root_idx| {
