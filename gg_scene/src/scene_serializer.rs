...
use gg_renderer::{ProjectionType, SceneCamera};
use crate::entity::Entity;
#[cfg(feature = "lua-scripting")]
use crate::LuaScriptComponent;
use crate::{
...
fn write_yaml_to_file<T: Serialize>(data: &T, file_path: &str, label: &str) -> EngineResult<()> {
    ...
    gg_core::platform_utils::atomic_write(file_path, &yaml)?;
...
impl SceneSerializer {
    ...
    pub fn restore_entity_from_json(scene: &mut Scene, uuid: u64, json: &str) -> EngineResult<()> {
        ...
        let entity = scene
            ...
            .ok_or_else(|| gg_core::error::EngineError::Asset(
                ...
                .map(|mc| {
                    ...
                    MeshRendererData {
                        ...
                        blend_mode: match mc.blend_mode {
                            gg_renderer::BlendMode::Opaque => "Opaque",
                            gg_renderer::BlendMode::AlphaBlend => "AlphaBlend",
                            gg_renderer::BlendMode::Additive => "Additive",
                        }
                        .to_string(),
                    }
                }),
            ...
            scene.add_component(
                ...
                MeshRendererComponent {
                    ...
                    blend_mode: match mrd.blend_mode.as_str() {
                        "AlphaBlend" => gg_renderer::BlendMode::AlphaBlend,
                        "Additive" => gg_renderer::BlendMode::Additive,
                        _ => gg_renderer::BlendMode::Opaque,
                    },
                    vertex_array: None,
                },
            );
            ...
            if sad.mesh_asset != 0 {
                ...
                for ce in &sad.clip_events {
                    let events = ce
                        ...
                        .iter()
                        .map(|e| {
                            gg_renderer::skeleton::SkeletalAnimationEvent {
                                time: e.time,
                                name: e.name.clone(),
                            }
                        })
...
mod tests {
    ...
    fn round_trip_serialize_deserialize() {
        ...
        sprite.texture_handle = gg_core::uuid::Uuid::from_raw(12345);
    ...
    fn tilemap_round_trip() {
        ...
        let tilemap = crate::TilemapComponent {
            width: 3,
            height: 2,
            ...
            texture_handle: gg_core::uuid::Uuid::from_raw(99999),
            ..Default::default()
        };
        ...
        let tm = loaded
            .get_component::<crate::TilemapComponent>(*ent)
            .unwrap();
    ...
    fn audio_source_round_trip() {
        ...
        let audio = crate::AudioSourceComponent {
            audio_handle: gg_core::uuid::Uuid::from_raw(77777),
            volume: 0.75,
            pitch: 1.2,
            looping: true,
            play_on_start: true,
            ...
            min_distance: 2.0,
            max_distance: 30.0,
            category: crate::AudioCategory::Music,
            priority: 200,
            resolved_path: None,
        };
        ...
        let ac = loaded
            .get_component::<crate::AudioSourceComponent>(*ent)
            .unwrap();
        ...
        assert_eq!(ac.category, crate::AudioCategory::Music);
    ...
    fn showcase_3d_audio_deserializes() {
        ...
        let (spatial_ent, _) = entities
            ...
            .unwrap();
        let ac3 = scene
            .get_component::<crate::AudioSourceComponent>(*spatial_ent)
            .unwrap();
        assert!(ac3.spatial);
        ...
        let (stream_ent, _) = entities
            ...
            .unwrap();
        let ac4 = scene
            .get_component::<crate::AudioSourceComponent>(*stream_ent)
            .unwrap();
        ...
        let listener = scene
            .get_component::<crate::AudioListenerComponent>(*cam_ent)
            .unwrap();
    ...
    fn prefab_round_trip() {
        ...
        sprite.texture_handle = gg_core::uuid::Uuid::from_raw(42);
...
"#;
    ...
    fn showcase_ui_scene_deserializes() {
        ...
        let rect = scene
            .get_component::<crate::UIRectComponent>(*btn)
            .unwrap();
        assert!((rect.size.x - 320.0).abs() < 0.01);
        assert!((rect.size.y - 60.0).abs() < 0.01);
        assert!(rect.raycast_target);
        assert!(scene.has_component::<crate::UIImageComponent>(*btn));
        assert!(scene.has_component::<crate::UIInteractableComponent>(*btn));
        assert!(scene.has_component::<crate::TextComponent>(*btn));

        // Panel has UIRect + UIImage + UILayout but no UIInteractable.
        let (panel, _) = entities.iter().find(|(_, n)| n == "Panel").unwrap();
        assert!(scene.has_component::<crate::UIRectComponent>(*panel));
        assert!(scene.has_component::<crate::UIImageComponent>(*panel));
        assert!(scene.has_component::<crate::UILayoutComponent>(*panel));
        assert!(!scene.has_component::<crate::UIInteractableComponent>(*panel));

        // Button is a child of Panel.
        let rel = scene
            .get_component::<crate::RelationshipComponent>(*btn)
            .unwrap();
