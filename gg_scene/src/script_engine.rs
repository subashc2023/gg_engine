...
impl ScriptFieldValue {
    ...
    pub fn from_lua_value(value: &LuaValue) -> Option<Self> {
        match value {
...
pub const ENTITY_ENVS_REGISTRY_KEY: &str = "__gg_entity_envs";
...
pub struct CurrentEntityUuid(pub u64);
...
pub struct PendingTimerOps {
    pub creates: Vec<PendingTimerCreate>,
    pub cancels: Vec<usize>,
    pub next_id: usize,
}

pub struct PendingTimerCreate {
    pub id: usize,
    pub entity_uuid: u64,
    pub delay: f32,
    pub repeating: bool,
...
pub struct PendingEventRegister {
    pub event_name: String,
    pub entity_uuid: u64,
    pub callback_key: LuaRegistryKey,
}
...
pub struct ScriptEngine {
    ...
    pub timers: HashMap<usize, ScriptTimer>,
    ...
    pub coroutines: Vec<ScriptCoroutine>,
...
impl ScriptEngine {
    ...
    pub fn create_entity_env(&mut self, uuid: u64, script_path: &str) -> bool {
        let _timer = gg_core::profiling::ProfileTimer::new("ScriptEngine::create_entity_env");
    ...
    /// Call a named callback with two string arguments
    /// (e.g. `on_animation_event(event_name, clip_name)`).
    pub fn call_entity_callback_str2(
        &mut self,
        uuid: u64,
        callback_name: &str,
        arg1: String,
        ...
        self.call_entity_function(uuid, callback_name, (arg1, arg2))
    }
