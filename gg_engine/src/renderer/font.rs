...
impl Font {
    ...
    pub(crate) fn load(res: &RendererResources<'_>, path: &Path) -> Self {
        let _timer = ProfileTimer::new("Font::load");

        let font_data =
            std::fs::read(path).unwrap_or_else(|e| panic!("Failed to read font '{}': {e}", path.display()));

        let face = ttf_parser::Face::parse(&font_data, 0)
            .unwrap_or_else(|e| panic!("Failed to parse font '{}': {e}", path.display()));
        ...
        struct MsdfGlyph {
            ...
            glyph_w: f64,
            glyph_h: f64,
            bitmap: Option<Vec<u8>>, // RGBA8 pixels, MSDF_GLYPH_SIZE x MSDF_GLYPH_SIZE
        }
        ...
        for &ch in &chars {
            ...
            // Autoframe: compute scale + translate to fit shape into bitmap.
            let frame = msdf::autoframe(&shape, MSDF_GLYPH_SIZE, MSDF_GLYPH_SIZE, MSDF_RANGE_PX);
            if frame.is_none() {
                // Degenerate shape — treat as whitespace.
                msdf_glyphs.push(MsdfGlyph {
                    ch,
                    advance_x,
                    bearing_x: 0.0,
            ...
            // Compute glyph bounds for metrics.
        ...
        for glyph in &msdf_glyphs {
            if let Some(ref bitmap_data) = glyph.bitmap {
                ...
                // Whitespace glyph: no bitmap, just advance.
                glyphs.insert(
                    glyph.ch,
                    GlyphInfo {
                        tex_coords: [[0.0; 2]; 4],
        ...
        log::info!(
            "Font loaded: '{}' — {} glyphs, {} kerning pairs, atlas {}x{} (MSDF)",
            path.display(),
            glyphs.len(),
            kerning_pairs.len(),
            atlas_width,
